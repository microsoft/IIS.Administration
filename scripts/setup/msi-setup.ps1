...
function Install() {
    ...
    .\install.ps1 -Path $adminRoot -Port $Port -DistributablePath $Path -Version $Version -ServiceName $ServiceName -IncludeDefaultCors:$IncludeDefaultCors -DontCopy -skipServiceStartupCheck:$skipServiceStartupCheck
}
...
function Upgrade() {
    ...
    $installed = $false
    try {
        .\install.ps1 -Path $adminRoot -Port 0 -DistributablePath $Path -Version $Version -ServiceName $ServiceName -IncludeDefaultCors:$IncludeDefaultCors -DontCopy -skipServiceStartupCheck:$skipServiceStartupCheck
        $installed = $true
