...
namespace Tests
{
    ...
    public class ApplicationPools
    {
        ...
        public static readonly string APP_POOLS_URL = $"{Globals.TEST_SERVER}:{Globals.TEST_PORT}/api/webserver/application-pools";
        ...
        public static JObject GetAppPool(HttpClient client, string poolName)
        {
            ...
            string poolContent;
            if (client.Get($"{ Globals.TEST_SERVER }:{ Globals.TEST_PORT }{ poolRef["_links"]["self"].Value<string>("href") }", out poolContent)) {
