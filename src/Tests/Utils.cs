...
namespace Tests
{
    ...
    public class Utils
    {
        ...
        public static JObject FollowLink(HttpClient client, JObject obj, string linkName)
        {
            ...
            string content;
            if (!client.Get($"{ Globals.TEST_SERVER }:{ Globals.TEST_PORT }{ href }", out content)) {
                return null;
            }
        ...
        {
            string link = $"{ Globals.TEST_SERVER }:{ Globals.TEST_PORT }{ obj["_links"][linkName].Value<string>("href") }";
        ...
        public static void InitializeTestEnvironment(string rootPath = Globals.TEST_ROOT_PATH)
        {
