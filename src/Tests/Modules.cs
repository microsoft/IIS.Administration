...
namespace Tests
{
    ...
    {
        public static readonly string GLOBAL_MODULES_URL = $"{Globals.TEST_SERVER}:{Globals.TEST_PORT}/api/webserver/global-modules";
        public static readonly string MODULES_URL = $"{Globals.TEST_SERVER}:{Globals.TEST_PORT}/api/webserver/http-modules";
        ...
        public static void DeleteModule(HttpClient client, JObject modulesFeature, string name)
        {
            if(modulesFeature != null) { 
                ...
                var entries = modulesRep.Value<JArray>("entries");

                JObject targetModule = null;
                foreach(JObject o in entries)
                {
                    if(o.Value<string>("name").Equals(name, StringComparison.OrdinalIgnoreCase))
                    {
                        targetModule = o;
                    }
                }
            ...
            var globalModules = GetGlobalModulesFeature(client).Value<JArray>("global_modules");

            JObject targetGlobalModule = null;
            foreach (JObject o in globalModules)
            {
                if (o.Value<string>("name").Equals(name, StringComparison.OrdinalIgnoreCase))
                {
                    targetGlobalModule = o;
                }
            }
