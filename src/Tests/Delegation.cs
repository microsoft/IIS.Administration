...
namespace Tests
{
    ...
    public class Delegation
    {
        ...
        public void SectionLocked()
        {
            ...
            using (HttpClient client = ApiHttpClient.Create($"{Globals.TEST_SERVER}:{Globals.TEST_PORT}")) {
                ...
                foreach (DelegatableFeature f in features) {
                    ...
                    // level is deny
                    var response = client.GetAsync($"{Globals.TEST_SERVER}:{Globals.TEST_PORT}{f.Path}?scope={TEST_SITE_NAME}").Result;
        ...
        {
            string delegationSectionsUri = $"{Globals.TEST_SERVER}:{Globals.TEST_PORT}/api/webserver/feature-delegation";
        ...
        private static JObject GetScopedFeature(HttpClient client, DelegatableFeature feature, string siteName, string path)
        {
            ...
            string content;
            if (!client.Get($"{Globals.TEST_SERVER}:{Globals.TEST_PORT}" + feature.Path + "?scope=" + siteName + path, out content)) {
                return null;
            }
